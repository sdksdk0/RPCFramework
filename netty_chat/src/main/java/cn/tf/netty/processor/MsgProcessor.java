package cn.tf.netty.processor;

import cn.tf.netty.protocol.IMDecoder;
import cn.tf.netty.protocol.IMEncoder;
import cn.tf.netty.protocol.IMMessage;
import cn.tf.netty.protocol.IMP;
...
public class MsgProcessor {
	...
	public static final AttributeKey<JSONObject> ATTRS = AttributeKey.valueOf("attrs"); //属性
	public static final AttributeKey<String> FROM = AttributeKey.valueOf("from"); //终端类型
	...
	public void sendMsg(Channel client,String msg){
		...
		if(request.getCmd().equals(IMP.LOGIN.getName())){
			//保存这个用户的ip地址，端口，昵称，终端类型，把这个用户保存到一个统一的容器中
